          ...
          <div class="mr-auto float-left bookmark-wrapper d-flex align-items-center">
            ...
            <ul class="nav navbar-nav bookmark-icons">
              @php
                ...
                $punto = DB::table('puntos_users')
                        ->join('puntos','puntos.id','=','puntos_users.puntos_id')
                ...
                if(count($role)>0){$role_id = $role[0]->role_id;}else{$role_id=0;}      
              @endphp
              @if($role_id == 2 || $role_id == 3)
                <li class="nav-item d-none d-lg-block">
                  
                  <b>Ubicación:</b>@if(count($punto)>0)<span style="color:#28c76f"> {{$punto[0]->nombre_punto}} - {{$punto[0]->ubicacion}}</span> @else<span style="color:red"> Sin ubicación </span>@endif
            ...
            <li class="dropdown dropdown-user nav-item"><a class="dropdown-toggle nav-link dropdown-user-link" href="#"
                ...
                <div class="user-nav d-sm-flex d-none"><span class="user-name text-bold-600">
                    @auth
                      {{ auth()->user()->name}}
                    @endauth
                    </span><span class="user-status" style="color:#28c76f">Conectado</span></div><span><img class="round"
                    src="{{asset('images/portrait/small/user_default.jpg') }}" alt="avatar" height="40"
                    width="40" /></span>
              </a>
              <div class="dropdown-menu dropdown-menu-right">
                <div class="dropdown-divider"></div>
                {{-- reemplazar la etiqueta a --}}
                
                <a class="dropdown-item" href="{{URL::to('formulario/cambiarpc')}}">
                  <i class="fa fa-street-view"></i> Cambiar punto control
                </a>
